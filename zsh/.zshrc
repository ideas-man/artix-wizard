...
source_conda ()
{
    ...
    __conda_setup="$('/mnt/volume/conda/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
    if [ $? -eq 0 ]; then
        eval "$__conda_setup"
    else
        if [ -f "/mnt/volume/conda/anaconda3/etc/profile.d/conda.sh" ]; then
            . "/mnt/volume/conda/anaconda3/etc/profile.d/conda.sh"
        else
            export PATH="/mnt/volume/conda/anaconda3/bin:$PATH"
        fi
    fi
    unset __conda_setup
    # <<< conda initialize <<<
